...
const RealEstateRequestsImportModal = ({ onClose, onImport, isRTL }) => {
  ...
  const generateTemplate = () => {
    const templateData = [
      {
        'Customer Name': isRTL ? 'اسم العميل' : 'Customer Name',
        'Project Name': isRTL ? 'اسم المشروع' : 'Project Name',
        'Unit Number': 'A-101',
    ...
    XLSX.writeFile(workbook, 'real_estate_requests_template.xlsx')
  }
        ...
        try {
          ...
          requiredFields.forEach(field => {
            const found = headers.some(header => 
              ...
              (header.includes('customer') && field === 'customer name') ||
              (header.includes('اسم العميل') && field === 'customer name') ||
              (header.includes('project') && field === 'project name') ||
              (header.includes('اسم المشروع') && field === 'project name')
            )
            if (!found) {
    ...
    try {
      ...
      reader.onload = (e) => {
        ...
        const mappedData = json.map(row => {
          ...
          return {
            customer: row[findKey(['Customer Name', 'Customer', 'اسم العميل'])] || '',
            project: row[findKey(['Project Name', 'Project', 'اسم المشروع'])] || '',
            unit: row[findKey(['Unit Number', 'Unit', 'رقم الوحدة'])] || '',
            type: row[findKey(['Request Type', 'Type', 'نوع الطلب'])] || 'Booking',
            status: row[findKey(['Status', 'الحالة'])] || 'Pending',
            date: row[findKey(['Date', 'التاريخ'])] || new Date().toISOString().split('T')[0],
            notes: row[findKey(['Notes', 'ملاحظات'])] || ''
          }
        }).filter(item => item.customer && item.project)
        ...
        <div className="px-6 py-6 overflow-y-auto custom-scrollbar">
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs" style={{ color: isDark ? '#d1d5db' : '#4b5563' }}>
                    {isRTL ? 'استخدم هذا النموذج لإضافة طلبات جديدة' : 'Use this template to add new requests'}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="mt-3 text-xs" style={{ color: isDark ? '#9ca3af' : '#6b7280' }}>
              <strong>{isRTL ? 'الحقول المطلوبة: ' : 'Required Fields: '}</strong> Customer Name, Project Name
            </div>
          </div>
          ...
          <div className="mt-3 text-xs" style={{ color: isDark ? '#9ca3af' : '#6b7280' }}>
            {isRTL ? 'الحقول المدعومة: اسم العميل، اسم المشروع، رقم الوحدة، نوع الطلب، الحالة، التاريخ، ملاحظات' : 'Supported Fields: Customer Name, Project Name, Unit Number, Request Type, Status, Date, Notes'}
          </div>
        </div>
      </div>
    </div>
...
export default RealEstateRequestsImportModal
