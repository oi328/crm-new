...
const generateDurationKPIs = (range, t) => {
  const multiplier = range === '7d' ? 0.8 : range === '30d' ? 1 : 1.2;
  return [
    { 
      title: t('Avg. Campaign Duration'), 
      value: `${Math.floor(24 * multiplier)} ${t('Days')}`, 
      change: +(5.2 * (range === '7d' ? 1.5 : 1)).toFixed(1), 
      trend: [20, 21, 22, 23, 24, 24, 25, 24, 24, 23, 24, 24].map(v => v * multiplier) 
    },
    { 
      title: t('Optimal Duration'), 
      value: `${Math.floor(14 * multiplier)}-${Math.floor(21 * multiplier)} ${t('Days')}`, 
      change: 0, 
      trend: [14, 14, 15, 15, 16, 18, 18, 19, 20, 21, 21, 21].map(v => v * multiplier),
      note: t('Based on peak ROAS')
    },
    { 
      title: t('Fatigue Rate'), 
      value: `${Math.floor(18 / multiplier)}%`, 
      change: -2.5, // Improvement
      isInverse: true,
      trend: [25, 24, 22, 21, 20, 19, 19, 18, 18, 18, 18, 18].map(v => v / multiplier) 
    },
    { 
      title: t('Long-Running ROI'), 
      value: `${(3.2 * multiplier).toFixed(1)}x`, 
      change: -8.4, 
      trend: [4.5, 4.2, 4.0, 3.8, 3.6, 3.5, 3.4, 3.3, 3.2, 3.2, 3.2, 3.2].map(v => v * multiplier) 
    }
  ]
}

const generatePerformanceCurve = (range, t) => {
  const multiplier = range === '7d' ? 0.9 : range === '90d' ? 1.1 : 1;
  return [
    { day: t('Day 1-3'), roas: +(1.8 * multiplier).toFixed(2), cpa: Math.floor(25 / multiplier), ctr: +(1.2 * multiplier).toFixed(1) },
    { day: t('Day 4-7'), roas: +(2.5 * multiplier).toFixed(2), cpa: Math.floor(18 / multiplier), ctr: +(1.8 * multiplier).toFixed(1) },
    { day: t('Day 8-14'), roas: +(3.8 * multiplier).toFixed(2), cpa: Math.floor(12 / multiplier), ctr: +(2.4 * multiplier).toFixed(1) }, // Peak
    { day: t('Day 15-21'), roas: +(3.5 * multiplier).toFixed(2), cpa: Math.floor(14 / multiplier), ctr: +(2.1 * multiplier).toFixed(1) },
    { day: t('Day 22-30'), roas: +(2.9 * multiplier).toFixed(2), cpa: Math.floor(19 / multiplier), ctr: +(1.7 * multiplier).toFixed(1) },
    { day: t('Day 30-60'), roas: +(2.2 * multiplier).toFixed(2), cpa: Math.floor(24 / multiplier), ctr: +(1.3 * multiplier).toFixed(1) },
    { day: t('Day 60+'), roas: +(1.5 * multiplier).toFixed(2), cpa: Math.floor(32 / multiplier), ctr: +(0.9 * multiplier).toFixed(1) },
  ]
}

const generateAgeDistribution = (range, t) => {
  const multiplier = range === '7d' ? 0.5 : range === '90d' ? 2 : 1;
  return [
    { bucket: t('< 1 Week'), count: Math.floor(8 * multiplier), spend: 5000 * multiplier },
    { bucket: t('1-2 Weeks'), count: Math.floor(12 * multiplier), spend: 12000 * multiplier },
    { bucket: t('2-4 Weeks'), count: Math.floor(6 * multiplier), spend: 8000 * multiplier },
    { bucket: t('1-3 Months'), count: Math.floor(4 * multiplier), spend: 15000 * multiplier },
    { bucket: t('> 3 Months'), count: Math.floor(2 * multiplier), spend: 4000 * multiplier },
  ]
}

const generateCampaignsHealth = (range) => {
  const multiplier = range === '7d' ? 0.8 : range === '90d' ? 1.2 : 1;
  return [
    { id: 1, name: 'Summer Sale Promo', platform: 'meta', duration: Math.floor(45 * multiplier), status: 'Fatigued', roas: (1.9 * multiplier).toFixed(1), ctr_trend: -15, spend: 5400 * multiplier },
    { id: 2, name: 'Search - Brand Keywords', platform: 'google', duration: Math.floor(120 * multiplier), status: 'Stable', roas: (4.68 * multiplier).toFixed(2), ctr_trend: 2, spend: 3200 * multiplier },
    { id: 3, name: 'TikTok Viral Video #1', platform: 'tiktok', duration: Math.floor(12 * multiplier), status: 'Peak', roas: (3.5 * multiplier).toFixed(1), ctr_trend: 10, spend: 1200 * multiplier },
    { id: 4, name: 'Retargeting - Cart Abandon', platform: 'meta', duration: Math.floor(28 * multiplier), status: 'Declining', roas: (2.9 * multiplier).toFixed(1), ctr_trend: -5, spend: 2100 * multiplier },
    { id: 5, name: 'Display - Awareness', platform: 'google', duration: Math.floor(5 * multiplier), status: 'Learning', roas: (1.1 * multiplier).toFixed(1), ctr_trend: 5, spend: 1500 * multiplier },
  ]
}
...
const CampaignDurationReport = () => {
  ...
  const durationKPIs = useMemo(() => generateDurationKPIs(timeRange, t), [timeRange, t])
  const performanceCurveData = useMemo(() => generatePerformanceCurve(timeRange, t), [timeRange, t])
  const ageDistributionData = useMemo(() => generateAgeDistribution(timeRange, t), [timeRange, t])
  const campaignsHealthData = useMemo(() => generateCampaignsHealth(timeRange), [timeRange])
