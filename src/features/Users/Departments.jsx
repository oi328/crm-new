...
import UserManagementDepartmentForm from './DepartmentForm';
...
const MOCK_DEPARTMENTS = [
  ...
  { id: 'd-3004', name: 'Marketing', manager: 'Nour El-Din', teamsCount: 2, employeesCount: 12, createdAt: '2025-06-15', status: 'Active' },
  { id: 'd-3005', name: 'Operations', manager: 'Khaled Omar', teamsCount: 5, employeesCount: 30, createdAt: '2025-05-20', status: 'Active' },
  { id: 'd-3006', name: 'Finance', manager: 'Mona Zaki', teamsCount: 2, employeesCount: 8, createdAt: '2025-04-10', status: 'Active' },
  { id: 'd-3007', name: 'Human Resources', manager: 'Amira Ahmed', teamsCount: 1, employeesCount: 5, createdAt: '2025-03-05', status: 'Active' },
  { id: 'd-3008', name: 'Legal', manager: 'Youssef Kamal', teamsCount: 1, employeesCount: 3, createdAt: '2025-02-28', status: 'Active' },
  { id: 'd-3009', name: 'IT', manager: 'Tarek Mahmoud', teamsCount: 3, employeesCount: 15, createdAt: '2025-01-15', status: 'Active' },
  { id: 'd-3010', name: 'Warehouse', manager: 'Sameh Hussein', teamsCount: 2, employeesCount: 20, createdAt: '2024-12-01', status: 'Active' },
  { id: 'd-3011', name: 'Procurement', manager: 'Hoda Ezzat', teamsCount: 1, employeesCount: 4, createdAt: '2024-11-20', status: 'Inactive' },
  { id: 'd-3012', name: 'Logistics', manager: 'Ramy Adel', teamsCount: 3, employeesCount: 18, createdAt: '2024-10-10', status: 'Active' },
];
...
export default function UserManagementDepartments() {
  ...
  const managers = useMemo(() => Array.from(new Set(departments.map(d => d.manager))), [departments]);
  ...
  const filtered = useMemo(() => {
    return departments.filter(d => {
      ...
      if (q) {
        ...
        const matchesSearch = [d.name, d.manager, d.id]
          .join(' ')
          .toLowerCase()
          .includes(query);
      ...
      if (filters.manager && filters.manager.length > 0) {
        if (!filters.manager.includes(d.manager)) return false;
      }
  ...
  const handleAssignSubmit = (payload) => {
    console.log('Assignment payload (Departments):', { ...payload, selectedDeptIds });
            ...
            <tbody>
              {sortedAndPaginated.map((d) => (
                <tr key={d.id} className="hover:bg-gray-800/50 dark:hover:bg-gray-800/50 transition-colors border-b border-gray-100 dark:border-gray-800 last:border-0">
                  ...
                  <td className="p-3 text-sm">{d.manager}</td>
                  <td className="p-3 text-sm">{d.teamsCount}</td>
                  <td className="p-3 text-sm">{d.employeesCount}</td>
                 ...
                 <div className="pr-8 rtl:pr-0 rtl:pl-8">
                   <div className="flex items-start justify-between mb-2">
                     <div>
                       ...
                       <span className="text-xs text-[var(--muted-text)] font-mono">{d.id}</span>
                     </div>
             ...
             <div className="w-full max-w-5xl h-[90vh] flex flex-col relative animate-in fade-in zoom-in-95 duration-200">
               <UserManagementDepartmentForm 
                 ...
                 onSuccess={(updatedDept) => {
                   const deptWithId = { ...updatedDept, id: updatedDept.code || selectedDepartment.id };
                   setDepartments(prev => prev.map(d => d.id === selectedDepartment.id ? { ...d, ...deptWithId } : d));
