...
export default function CreatePropertyModal({ onClose, isRTL, onSave, isEdit, buildings = [], owners = [], initialData = null }) {
  ...
  const categoryOptions = React.useMemo(() => {
    // Mock categories based on project
    if (formData.project === 'Palm Hills') return ['Residential', 'Commercial']
    if (formData.project === 'Mountain View') return ['Residential', 'Administrative']
    return ['Residential', 'Commercial', 'Administrative', 'Medical', 'Coastal']
  }, [formData.project])
  ...
  useEffect(() => {
    if (formData.totalPrice && !formData.price) {
  ...
  const validateStep = (step) => {
    ...
    if (step === 6) {
      ...
      if (!formData.cilSubject.trim() && !formData.cilSubjectAr.trim()) newErrors.cilSubject = 'Subject is required'
    }
  ...
  const validateAll = () => {
    ...
    return step1Valid && step5Valid && step6Valid
  }
  ...
  const handleFinish = () => {
    if (validateAll()) {
      ...
      onClose()
       ...
       else if (!validateStep(6)) setCurrentStep(6)
    }
  }
  ...
  const applyPlanTemplate = (plan) => {
    ...
    const years = freq.includes('quarter') ? inst / 4 : freq.includes('semi') ? inst / 2 : inst / 12
    const nowYear = new Date().getFullYear()
    const delivery = years ? String(Math.ceil(nowYear + years)) : ''
    setFormData(prev => ({
      ...prev,
      installmentPlans: [
        { 
  ...
  const renderStep3 = () => (
    <div className="space-y-6 animate-fadeIn">
      {/* Images */}
      <div className="space-y-2">
        <label className="label">{inputLanguage === 'ar' ? 'صور العقار' : 'Property Images'}</label>
        <FileUploader 
          label={inputLanguage === 'ar' ? 'اسحب وأفلت الصور هنا' : 'Drag & Drop Photos Here'}
