...
const ItemsImportModal = ({ onClose, onImport, isRTL }) => {
  ...
  const generateTemplate = () => {
    const templateData = [
      {
        'Item Name': isRTL ? 'مثال للصنف' : 'Example Item',
        'SKU': 'ITEM-001',
        'Family': 'Electronics',
        'Category': 'Mobiles',
        'Group': 'Smartphones',
        ...
        'Unit': 'pcs',
        'Status': 'Active',
        'Description': isRTL ? 'وصف الصنف' : 'Item Description'
      }
    ]
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const requiredFields = ['item name'] // or 'name'
          const missingFields = []

          requiredFields.forEach(field => {
            const found = headers.some(header => 
              header.includes(field) || 
              (header.includes('name') && field === 'item name') ||
              (header.includes('اسم') && field === 'item name')
            )
            if (!found) {
    ...
    try {
      ...
      reader.onload = (e) => {
        ...
        const mappedData = json.map(row => {
          ...
          return {
            name: row[findKey(['Item Name', 'Name', 'اسم'])] || '',
            sku: row[findKey(['SKU', 'كود'])] || '',
            family: row[findKey(['Family', 'العائلة'])] || '',
            category: row[findKey(['Category', 'التصنيف'])] || '',
            group: row[findKey(['Group', 'المجموعة'])] || '',
            brand: row[findKey(['Brand', 'العلامة'])] || '',
            supplier: row[findKey(['Supplier', 'المورد'])] || '',
            type: row[findKey(['Type', 'النوع'])] || 'Product',
            price: Number(row[findKey(['Price', 'السعر'])]) || 0,
            stock: Number(row[findKey(['Stock', 'المخزون'])]) || 0,
            minStock: Number(row[findKey(['Min Stock', 'الحد الأدنى'])]) || 0,
            unit: row[findKey(['Unit', 'الوحدة'])] || 'pcs',
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold" style={{ color: isDark ? 'white' : '#111827' }}>{isRTL ? 'استيراد أصناف' : 'Import Items'}</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6 overflow-y-auto custom-scrollbar">
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs" style={{ color: isDark ? '#d1d5db' : '#4b5563' }}>
                    {isRTL ? 'استخدم هذا النموذج لإضافة أصناف جديدة' : 'Use this template to add new items'}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="mt-3 text-xs" style={{ color: isDark ? '#9ca3af' : '#6b7280' }}>
              <strong>{isRTL ? 'الحقول المطلوبة: ' : 'Required Fields: '}</strong> Item Name
            </div>
          </div>
          ...
          {importSummary && (
            <div className="mt-4 px-4 py-3 rounded-lg bg-green-50 text-green-700 border border-green-200 dark:bg-green-900/50 dark:text-green-200 dark:border-green-800">
              {isRTL ? `تم استيراد ${importSummary.added} صنف بنجاح` : `Successfully imported ${importSummary.added} items`}
            </div>
          )}

          <div className="mt-3 text-xs" style={{ color: isDark ? '#9ca3af' : '#6b7280' }}>
            {isRTL ? 'الحقول المدعومة: اسم الصنف، SKU، العائلة، التصنيف، المجموعة، العلامة، السعر، المخزون...' : 'Supported Fields: Item Name, SKU, Family, Category, Group, Brand, Price, Stock...'}
          </div>
        </div>
      </div>
    </div>
