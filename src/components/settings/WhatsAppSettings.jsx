...
export default function WhatsAppSettings() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('wa-settings')
    if (saved) {
      try {
        ...
        setConnStatus(s.connStatus || 'disconnected')
        setMonthlyCount(s.monthlyCount || 0)
        setTemplates(s.templates || [])
        setTriggers(s.triggers || triggers)
        setAutoReplies(s.autoReplies || autoReplies)
      } catch {}
    }
  }, [])
  ...
  const saveAll = () => {
    const payload = {
      provider, apiKey, apiSecret, businessNumber, businessId, webhookUrl,
      connStatus, monthlyCount, templates, triggers, autoReplies,
    }
    localStorage.setItem('wa-settings', JSON.stringify(payload))
  ...
  const saveTemplate = () => {
    ...
    if (editingTpl) {
      setTemplates(prev => prev.map(t => t.id === editingTpl.id ? { ...t, name, category: tplCategory, language: tplLanguage, body, updatedAt: new Date().toLocaleString() } : t))
      setToast({ type:'success', message:'Template updated.' })
    } else {
      const id = 'wa-' + name.toLowerCase().replace(/\s+/g,'-') + '-' + Math.random().toString(36).slice(2)
      setTemplates(prev => [{ id, name, category: tplCategory, language: tplLanguage, status: 'Pending', body, updatedAt: new Date().toLocaleString() }, ...prev])
