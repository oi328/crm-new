...
const BrokersImportModal = ({ onClose, onImport, isRTL }) => {
  ...
  const generateTemplate = () => {
    const templateData = [
      {
        'Broker Name': isRTL ? 'اسم الوسيط' : 'Broker Name',
        'Agency Name': isRTL ? 'اسم الوكالة' : 'Agency Name',
        'Phone': '01000000000',
        'Email': 'broker@example.com',
        'Commission Rate': '2.5%',
        'Broker Type': isRTL ? 'فرد' : 'Individual', // Individual, Agency
        'Status': 'Active'
      }
    ]
    ...
    XLSX.writeFile(workbook, 'brokers_template.xlsx')
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const requiredFields = ['broker', 'phone']
          const missingFields = []

          requiredFields.forEach(field => {
            const found = headers.some(header => 
              header.includes(field) || 
              (header.includes('name') && field === 'broker') ||
              (header.includes('وسيط') && field === 'broker') ||
              (header.includes('هاتف') && field === 'phone') ||
              (header.includes('موبايل') && field === 'phone')
            )
            if (!found) {
              missingFields.push(field === 'broker' ? (isRTL ? 'اسم الوسيط' : 'Broker Name') : (isRTL ? 'رقم الهاتف' : 'Phone'))
            }
          })
    ...
    try {
      ...
      reader.onload = (e) => {
        ...
        const mappedData = json.map(row => {
          ...
          const type = row[findKey(['Type', 'نوع'])] || 'Individual'

          return {
            name: row[findKey(['Broker', 'وسيط', 'Name', 'الاسم'])] || '',
            agencyName: row[findKey(['Agency', 'وكالة'])] || '',
            phone: row[findKey(['Phone', 'هاتف', 'موبايل'])] || '',
            email: row[findKey(['Email', 'بريد'])] || '',
            commissionRate: row[findKey(['Commission', 'عمولة'])] || '',
            brokerType: type.toLowerCase() === 'agency' || type === 'وكالة' ? 'agency' : 'individual',
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold" style={{ color: isDark ? 'white' : '#111827' }}>
              {isRTL ? 'استيراد وسطاء' : 'Import Brokers'}
            </h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6 overflow-y-auto custom-scrollbar">
          ...
          >
            ...
            <div className="mt-3 text-xs" style={{ color: isDark ? '#9ca3af' : '#6b7280' }}>
              <strong>{isRTL ? 'الحقول المطلوبة: ' : 'Required Fields: '}</strong> Broker Name, Phone
            </div>
          </div>
          ...
          <div className="mt-3 text-xs" style={{ color: isDark ? '#9ca3af' : '#6b7280' }}>
            {isRTL ? 'الحقول المدعومة: اسم الوسيط، الوكالة، الهاتف، البريد، العمولة' : 'Supported Fields: Broker Name, Agency Name, Phone, Email, Commission'}
          </div>
        </div>
      </div>
    </div>
