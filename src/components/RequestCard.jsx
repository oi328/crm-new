...
export default function RequestCard({
  ...
  const isVisible = (key) => !visibleColumns || visibleColumns[key];
    ...
    <div dir={dir} className="bg-white dark:bg-gray-800 shadow rounded-lg p-4 flex flex-col gap-3 transition hover:shadow-md">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          {isVisible('id') && <span className="text-xs px-2 py-1 rounded bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200">#{request.id}</span>}
          {isVisible('customerName') && <span className="font-semibold text-gray-800 dark:text-gray-100">{request.customerName}</span>}
        </div>
        <span className="text-xs px-2 py-1 rounded border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300">
          {isVisible('type') && <span>{request.type}</span>}
          {isVisible('type') && isVisible('priority') && <span> • </span>}
          {isVisible('priority') && <span>{request.priority}</span>}
        </span>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
        {isVisible('propertyUnit') && (
