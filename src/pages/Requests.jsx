...
export default function Requests() {
  ...
  const handleImportRequests = (rows) => {
    ...
    mapped.forEach(r => saveRequest(r));
  };
                    ...
                    <tr key={r.id} className=" dark:hover:bg-transparent transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium min-w-[300px]">
                        <div className="flex items-center gap-2">
                          ...
                          {r.status !== 'Approved' && (
                          <button 
                            onClick={() => saveRequest({ ...r, status: 'Approved', updatedAt: new Date().toISOString() })}
                          ...
                          {r.status !== 'Rejected' && (
                          <button 
                            onClick={() => saveRequest({ ...r, status: 'Rejected', updatedAt: new Date().toISOString() })}
                     ...
                     <div key={req.id} className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow cursor-pointer group relative">
                        ...
                        <div className="flex items-center justify-between text-xs text-[var(--muted-text)] pt-2 border-t border-gray-100 dark:border-gray-700">
                           ...
                           <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              ...
                              <button onClick={(e)=>{e.stopPropagation(); deleteRequest(req.id)}} className="p-1 hover:bg-gray-100 rounded text-red-600"><FaTrash size={12} /></button>
                           </div>
                        </div>
                     </div>
                   ))}
        ...
        <AssignmentModal
          ...
          onSubmit={(payload) => {
            ...
            if (assignTargetRequest) {
              saveRequest({ ...assignTargetRequest, assignedTo: String(name), updatedAt: new Date().toISOString() });
            }
          }}
