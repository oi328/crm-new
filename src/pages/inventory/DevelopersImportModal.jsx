...
const DevelopersImportModal = ({ onClose, onImport, isRTL }) => {
  ...
  const generateTemplate = () => {
    const templateData = [
      {
        'Company Name': isRTL ? 'شركة التطوير' : 'Developer Company',
        'Contact Person': isRTL ? 'الشخص المسؤول' : 'Contact Person',
        'Phone': '01000000000',
        'Email': 'contact@example.com',
        'City': isRTL ? 'القاهرة' : 'Cairo',
        'Project Type': isRTL ? 'سكني' : 'Residential',
        'Status': 'Active'
      }
    ]
    ...
    XLSX.writeFile(workbook, 'developers_template.xlsx')
  }
        ...
        try {
          ...
          requiredFields.forEach(field => {
            const found = headers.some(header => 
              ...
              (header.includes('name') && field === 'company') ||
              (header.includes('شركة') && field === 'company')
            )
            if (!found) {
              missingFields.push(isRTL ? 'اسم الشركة' : 'Company Name')
            }
          })
    ...
    try {
      ...
      reader.onload = (e) => {
        ...
        const mappedData = json.map(row => {
          ...
          return {
            companyName: row[findKey(['Company', 'شركة'])] || '',
            contactPerson: row[findKey(['Contact', 'مسؤول'])] || '',
            phone: row[findKey(['Phone', 'هاتف', 'موبايل'])] || '',
            email: row[findKey(['Email', 'بريد'])] || '',
            city: row[findKey(['City', 'مدينة'])] || '',
            projectType: row[findKey(['Type', 'نوع'])] || '',
            status: row[findKey(['Status', 'حالة'])] || (isRTL ? 'نشط' : 'Active')
          }
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold" style={{ color: isDark ? 'white' : '#111827' }}>
              {isRTL ? 'استيراد مطورين' : 'Import Developers'}
            </h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6 overflow-y-auto custom-scrollbar">
          ...
          >
            ...
            <div className="mt-3 text-xs" style={{ color: isDark ? '#9ca3af' : '#6b7280' }}>
              <strong>{isRTL ? 'الحقول المطلوبة: ' : 'Required Fields: '}</strong> Company Name
            </div>
          </div>
          ...
          {importSummary && (
            <div className="mt-4 px-4 py-3 rounded-lg bg-green-50 text-green-700 border border-green-200 dark:bg-green-900/50 dark:text-green-200 dark:border-green-800">
              {isRTL ? `تم استيراد ${importSummary.added} مطور بنجاح` : `Successfully imported ${importSummary.added} developers`}
            </div>
          )}

          <div className="mt-3 text-xs" style={{ color: isDark ? '#9ca3af' : '#6b7280' }}>
            {isRTL ? 'الحقول المدعومة: اسم الشركة، المسؤول، الهاتف، البريد، المدينة' : 'Supported Fields: Company Name, Contact Person, Phone, Email, City'}
          </div>
        </div>
      </div>
    </div>
...
export default DevelopersImportModal
