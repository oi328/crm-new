...
const RealEstatePriceBooksImportModal = ({ onClose, onImport, isRTL }) => {
  ...
  const generateTemplate = () => {
    const templateData = [
      {
        'Name': isRTL ? 'قائمة أسعار عقارات' : 'Real Estate Price Book',
        'Currency': 'EGP',
        'Valid From': '2024-01-01',
        'Valid To': '2024-12-31',
        'Status': 'Active',
        'Description': isRTL ? 'قائمة أسعار 2024' : '2024 Price List'
      }
    ]

    const worksheet = XLSX.utils.json_to_sheet(templateData)
    const workbook = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Price Books Template')
    XLSX.writeFile(workbook, 'real_estate_price_books_template.xlsx')
  }
        ...
        try {
          ...
          requiredFields.forEach(field => {
            const found = headers.some(header => 
              ...
              (header.includes('book name') && field === 'name') ||
              (header.includes('اسم') && field === 'name')
            )
            if (!found) {
              missingFields.push(field === 'name' ? (isRTL ? 'اسم القائمة' : 'Name') : field)
            }
          })
    ...
    try {
      ...
      reader.onload = (e) => {
        ...
        const mappedData = json.map(row => {
          ...
          return {
            name: row[findKey(['Name', 'Book Name', 'اسم'])] || '',
            currency: row[findKey(['Currency', 'العملة'])] || 'EGP',
            validFrom: row[findKey(['Valid From', 'Start Date', 'تاريخ البدء'])] || null,
            validTo: row[findKey(['Valid To', 'End Date', 'تاريخ الانتهاء'])] || null,
            status: row[findKey(['Status', 'الحالة'])] || 'Active',
            description: row[findKey(['Description', 'الوصف'])] || ''
          }
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold" style={{ color: isDark ? 'white' : '#111827' }}>{isRTL ? 'استيراد قوائم أسعار العقارات' : 'Import Real Estate Price Books'}</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6 overflow-y-auto custom-scrollbar">
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs" style={{ color: isDark ? '#d1d5db' : '#4b5563' }}>
                    {isRTL ? 'استخدم هذا النموذج لإضافة قوائم جديدة' : 'Use this template to add new price books'}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="mt-3 text-xs" style={{ color: isDark ? '#9ca3af' : '#6b7280' }}>
              <strong>{isRTL ? 'الحقول المطلوبة: ' : 'Required Fields: '}</strong> Name
            </div>
          </div>
          ...
          {importSummary && (
            <div className="mt-4 px-4 py-3 rounded-lg bg-green-50 text-green-700 border border-green-200 dark:bg-green-900/50 dark:text-green-200 dark:border-green-800">
              {isRTL ? `تم استيراد ${importSummary.added} قائمة بنجاح` : `Successfully imported ${importSummary.added} price books`}
            </div>
          )}

          <div className="mt-3 text-xs" style={{ color: isDark ? '#9ca3af' : '#6b7280' }}>
            {isRTL ? 'الحقول المدعومة: الاسم، العملة، تاريخ البدء، تاريخ الانتهاء، الحالة، الوصف' : 'Supported Fields: Name, Currency, Valid From, Valid To, Status, Description'}
          </div>
        </div>
      </div>
    </div>
...
export default RealEstatePriceBooksImportModal
