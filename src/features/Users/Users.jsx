...
import { ClipboardCheck, Handshake, Ticket, Filter, ChevronDown, ChevronUp, Search, Calendar, ArrowUpDown, ChevronLeft, ChevronRight, X, Pencil, Key, ToggleLeft, ToggleRight, MoreVertical, Trash, Clock, UserCog, Monitor, Smartphone, Eye } from 'lucide-react';
...
import UserManagementUserCreate from './UserForm';
...
const mockUsers = [
  {
    ...
    devices: [
      { name: 'Chrome on Windows', ip: '192.168.1.105', type: 'Desktop', lastActive: '2025-11-18 10:24' },
      { name: 'Safari on iPhone', ip: '10.0.0.45', type: 'Mobile', lastActive: '2025-11-17 15:30' }
    ],
  },
  {
    id: 'u-1002',
    ...
    devices: [
      { name: 'Samsung Galaxy S21', ip: '192.168.1.12', type: 'Mobile', lastActive: '2025-11-15 09:17' }
    ],
  },
  {
    id: 'u-1003',
    ...
    devices: [
      { name: 'Firefox on MacOS', ip: '172.16.0.23', type: 'Desktop', lastActive: '2025-11-10 14:05' }
    ],
  },
];
...
const UserActions = ({ user, onEdit, onPreview, onChangePassword, onToggleActive, onDelete, onAssignRotation, onDelayRotation }) => {
  ...
  return (
    <div className={`flex items-center gap-2 relative min-w-[140px] ${showDropdown ? 'z-50' : ''}`} ref={dropdownRef}>
      <button 
        onClick={onPreview} 
        title="Preview" 
        className="p-1.5 rounded-md hover:bg-purple-100 dark:hover:bg-purple-900/40 transition-colors"
      >
        <Eye className="w-5 h-5 block shrink-0 text-purple-600" />
      </button>
      ...
      >
        <Pencil className="w-5 h-5 block shrink-0 text-blue-600" />
      </button>
      ...
      >
        <Key className="w-5 h-5 block shrink-0 text-amber-600" />
      </button>
...
export default function UserManagementUsers() {
  ...
  const handleChangePasswordSubmit = (userId, newPassword) => {
      ...
      <div className="glass-panel p-4 rounded-xl mb-6">
        <div className="flex justify-between items-center mb-3">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="px-3 py-1.5 text-sm bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-blue-900/20 dark:text-blue-400 dark:hover:bg-blue-900/40 rounded-lg transition-colors flex items-center gap-2"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="space-y-1">
            ...
            <SearchableSelect
              options={DEPARTMENTS ? DEPARTMENTS.map(o => ({ value: o.name, label: o.name })) : []}
          ...
          <div className="overflow-x-auto hidden md:block">
            ...
            <thead>
              <tr>
                ...
                <th className="p-3 cursor-pointer hover:text-blue-500 transition-colors" onClick={() => handleSort('username')}>
                   <div className="flex items-center gap-1">
                    {isArabic ? 'اسم المستخدم' : 'Username'}
                    <ArrowUpDown size={12} className={sortBy === 'username' ? 'text-blue-500' : 'opacity-30'} />
                   </div>
                </th>
                  ...
                  {sortedAndPaginated.map((u) => (
                    <tr key={u.id} className="group hover:bg-gray-800/50 dark:hover:bg-gray-800/50 transition-colors border-b border-gray-100 dark:border-gray-800 last:border-0">
                      ...
                      <td className="p-3">
                        <div className="flex items-center gap-3">
                          ...
                          <Link to={`/user-management/users/${u.id}`} className="font-medium hover:text-blue-600 transition-colors">
                            {u.fullName}
                            <div className="text-xs text-[var(--muted-text)] font-normal md:hidden">{u.email}</div>
                          </Link>
                        </div>
                      </td>
                      <td className="p-3 text-sm font-mono text-[var(--muted-text)]">
                      ...
                      <td className="p-3 text-sm">
                         <div className="flex items-center gap-1">
                           {u.devices?.map((d, i) => {
                              const deviceName = typeof d === 'string' ? d : d.name;
                              const deviceType = (typeof d === 'string' ? d : (d.type || d.name))?.toLowerCase() || '';
                              const isMobile = deviceType.includes('mobile') || deviceType.includes('phone') || deviceType.includes('android') || deviceType.includes('ios');
                              
                              return (
                                <div key={i} className="tooltip" data-tip={deviceName}>
                                  {isMobile ? <Smartphone size={16} className="text-[var(--muted-text)]"/> : <Monitor size={16} className="text-[var(--muted-text)]"/>}
                                </div>
                              );
               ...
               <div key={u.id} className=" p-4 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm relative group">
                 ...
                 <div className="flex items-start gap-3 pr-8 rtl:pr-0 rtl:pl-8">
                   ...
                   <div className="flex-1 min-w-0">
                     ...
                     <div className="text-xs text-[var(--muted-text)] font-mono truncate mb-1">
                       {u.email}
                     </div>
                 ...
                 <div className="mt-4 pt-3 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
                    <div className="flex items-center gap-1 text-[var(--muted-text)]">
                       {u.devices?.some(d => (typeof d === 'string' ? d : d.type)?.toLowerCase().includes('mobile')) 
                         ? <Smartphone size={14} /> 
                         : <Monitor size={14} />
                       }
