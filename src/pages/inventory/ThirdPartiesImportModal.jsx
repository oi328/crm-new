...
const ThirdPartiesImportModal = ({ onClose, onImport, isRTL }) => {
  ...
  const generateTemplate = () => {
    const templateData = [
      {
        'Name': isRTL ? 'شركة المثال' : 'Example Company',
        'Type': 'Supplier',
        'Contact Info': '+1234567890',
        'Description': isRTL ? 'وصف الكيان' : 'Entity Description'
      }
    ]

    const worksheet = XLSX.utils.json_to_sheet(templateData)
    const workbook = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Third Parties Template')
    XLSX.writeFile(workbook, 'third_parties_template.xlsx')
  }
        ...
        try {
          ...
          requiredFields.forEach(field => {
            const found = headers.some(header => 
              ...
              (header.includes('entity name') && field === 'name') ||
              (header.includes('اسم') && field === 'name')
            )
            if (!found) {
              missingFields.push(field === 'name' ? (isRTL ? 'اسم الكيان' : 'Name') : field)
            }
          })
    ...
    try {
      ...
      reader.onload = (e) => {
        ...
        const mappedData = json.map(row => {
          ...
          return {
            name: row[findKey(['Name', 'Entity Name', 'اسم'])] || '',
            type: row[findKey(['Type', 'Entity Type', 'النوع'])] || 'Supplier',
            contact_info: row[findKey(['Contact Info', 'Contact', 'Phone', 'اتصال'])] || '',
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold" style={{ color: isDark ? 'white' : '#111827' }}>{isRTL ? 'استيراد كيانات' : 'Import Entities'}</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6 overflow-y-auto custom-scrollbar">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs" style={{ color: isDark ? '#d1d5db' : '#4b5563' }}>
                    {isRTL ? 'استخدم هذا النموذج لإضافة كيانات جديدة' : 'Use this template to add new entities'}
                  </p>
                </div>
              </div>
              <button
          ...
          {importSummary && (
            <div className="mt-4 px-4 py-3 rounded-lg bg-green-50 text-green-700 border border-green-200 dark:bg-green-900/50 dark:text-green-200 dark:border-green-800">
              {isRTL ? `تم استيراد ${importSummary.added} كيان بنجاح` : `Successfully imported ${importSummary.added} entities`}
            </div>
          )}

          <div className="mt-3 text-xs" style={{ color: isDark ? '#9ca3af' : '#6b7280' }}>
            {isRTL ? 'الحقول المدعومة: الاسم، النوع، معلومات الاتصال، الوصف' : 'Supported Fields: Name, Type, Contact Info, Description'}
          </div>
        </div>
      </div>
    </div>
...
export default ThirdPartiesImportModal
