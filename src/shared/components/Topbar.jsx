          ...
          <div className="relative max-[768px]:hidden" ref={profileRef}>
            ...
            {profilePreviewOpen && (
              <div className={`dropdown-panel absolute top-12 ${isRTL ? 'left-0' : 'right-0'} w-64 max-w-[85vw] ${isLight ? 'bg-white border border-gray-200' : 'bg-gray-900 border border-gray-700'} rounded-xl shadow-2xl z-50`} role="menu" aria-label={t('Profile menu')}>
                ...
                <button
                  onClick={() => { navigate('/settings/system/security') }}
                ...
                <button
                  onClick={() => { navigate('/user-management/access-logs') }}
                  className="w-full flex items-center gap-3 px-4 py-2 text-sm text-[var(--content-text)] hover:bg-[var(--table-row-hover)] transition-colors"
                  role="menuitem"
                >
                  <span className="w-5 h-5 inline-flex items-center justify-center">📱</span>
                ...
                <button
                  onClick={async () => { await svcLogout(); navigate('/login') }}
                  className="w-full flex items-center gap-3 px-4 py-2 text-sm text-[var(--content-text)] hover:bg-[var(--table-row-hover)] transition-colors"
                  role="menuitem"
                >
                  <span className="w-5 h-5 inline-flex items-center justify-center">↪️</span>
    ...
    {isProfileMobileOpen && (
      <div ref={profileMobileRef} className="md:hidden fixed z-50" style={{ top: profileMenuPos.top, left: isRTL ? profileMenuPos.left : undefined, right: isRTL ? undefined : profileMenuPos.right }}>
        <div className={`dropdown-panel w-[260px] max-w-[90vw] ${isLight ? 'bg-white border border-gray-200' : 'bg-gray-900 border border-gray-700'} rounded-xl shadow-2xl`} role="menu" aria-label={t('Profile menu')}>
          ...
          <div className="h-px bg-[var(--divider)]"></div>
          <button onClick={() => { navigate('/settings/system/security') }} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-[var(--content-text)] hover:bg-[var(--table-row-hover)]" role="menuitem"><span className="w-5 h-5 inline-flex items-center justify-center">🔐</span><span>{t('Security Settings')}</span></button>
          <button onClick={() => { navigate('/user-management/access-logs') }} className="w-full flex items_center gap-2 px-3 py-2 text-xs text-[var(--content-text)] hover:bg-[var(--table-row-hover)]" role="menuitem"><span className="w-5 h-5 inline-flex items_center justify_center">📱</span><span>{t('Linked Devices')}</span></button>
          ...
          <div className="h-px bg-[var(--divider)]"></div>
          <button onClick={async () => { await svcLogout(); navigate('/login') }} className="w-full flex items_center gap-2 px-3 py-2 text-xs text-[var(--content_text)] hover:bg-[var(--table-row-hover)]" role="menuitem"><span className="w-5 h-5 inline-flex items_center justify_center">↪️</span><span>{t('Logout')}</span></button>
        </div>
      </div>
    )}
